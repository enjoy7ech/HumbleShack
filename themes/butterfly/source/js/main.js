document.addEventListener("DOMContentLoaded", function () {
  ...
  const addHighlightTool = function () {
    ...
    const doReplFn = (ele) => {
      if (ele.dataset.lang === "python" || ele.dataset.lang === "javascript")
        window.open(
          `https://repl.dongzx.lol/repl/index.html?kernel=${ele.dataset.lang}&toolbar=1&theme=JupyterLab%20Night`,
          "_blank",
        );
      else
        window.open(
          `https://repl.dongzx.lol/repl/index.html?toolbar=1&theme=JupyterLab%20Night`,
          "_blank",
        );
    };
    ...
    const highlightToolsFn = function (e) {
      ...
      else if ($target.contains("repl-button")) doReplFn(e.target);
    };
      ...
      if (isPrismjs) {
        $figureHighlight.forEach(function (item) {
          const langName = item.getAttribute("data-language")
            ...
            : "Code";
          const highlightLangEle = `<div class="code-lang">${langName}<i class="fas fa-code repl-button" data-lang="${langName}"></i></div>`;
